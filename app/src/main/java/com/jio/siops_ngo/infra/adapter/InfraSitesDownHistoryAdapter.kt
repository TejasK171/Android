...
import com.jio.siops_ngo.viewholder.InfraSitesDownHistoryViewHolder


class InfraSitesDownHistoryAdapter(
    private val datalist: ArrayList<Map<String, Any>>, private val activity: MainActivity?
) : RecyclerView.Adapter<InfraSitesDownHistoryViewHolder>() {

    var boolean: Boolean = true
    var dataListNgoItem: ArrayList<Map<String, Any>>? = null

    override fun onCreateViewHolder(parent: ViewGroup, viwType: Int): InfraSitesDownHistoryViewHolder {
        var view: View?


        view = LayoutInflater.from(parent.context)
            .inflate(R.layout.sites_down_history_item, parent, false)

        return InfraSitesDownHistoryViewHolder(view)
    }

    @SuppressLint("ResourceAsColor")
    override fun onBindViewHolder(holder: InfraSitesDownHistoryViewHolder, position: Int) {
        try {
            ...
            if(content.containsKey("impactedCustomers") && content.get("impactedCustomers")!=null){

                holder.txtIc!!.text = content["impactedCustomers"] as String
            }else{
                holder.txtIc!!.visibility = View.GONE
            }
