package com.jio.siops_ngo.businessBoard.adapter
...
import com.jio.siops_ngo.businessBoard.fragment.OrderActivationCircleFragment
...
class OrderActivationRegionAdapter(
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viwType: Int): OrderJouneryViewholder {
        ...
        view =
            LayoutInflater.from(parent.context).inflate(R.layout.journey_item_view, parent, false)
    ...
    override fun onBindViewHolder(holder: OrderJouneryViewholder, position: Int) {
        try {
            val activationData = appList[position]


            if (position != 0)
                holder.txtCircle!!.visibility = View.VISIBLE
            else
                holder.txtCircle!!.visibility = View.GONE

            if (activationData!!.containsKey("zone") && activationData.get("zone") != null) {
                if(position==0) {
                    holder.txtTitle!!.text =
                        activationData["zone"] as String + " (" + categoryToDisplay + ")"
                }else{
                    holder.txtTitle!!.text =
                        activationData["zone"] as String
                }
            }
...
//                holder.txtTvPendingCount!!.text = activationData["tv_pending"].toString()
            ...
            holder.txtCircle!!.setOnClickListener {
                val orderActivationCircleFragment = OrderActivationCircleFragment.newInstance()
                val commonBean = CommonBean()
                commonBean.`object` = activationData as HashMap<String, Any>
                orderActivationCircleFragment.setData(commonBean,holder.txtTitle!!.text.toString(), apiRequestFormattedDate!!)
                activity!!.openFragments(orderActivationCircleFragment, commonBean, true, true)
            }
